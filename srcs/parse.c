...
/*   Updated: 2020/10/14 13:28:43 by isel-jao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int parse(t_mlx *m, char *cub)
{
	...
	while (m->tab[i] && !ret)
	{
		if (m->tab[i][0] == 'R' && m->tab[i][1] == ' ')
			ret = ft_res(m, i);
		else if (!ft_strncmp(m->tab[i], "NO ", 3) || \
		!ft_strncmp(m->tab[i], "SO ", 3) ||!ft_strncmp(m->tab[i], "WE ", 3) || \
		!ft_strncmp(m->tab[i], "EA ", 3) || (m->tab[i][0] == 'S' && m->tab[i][1] == ' '))
			ret = ft_texture(m, i);
