...
/*   Updated: 2020/09/19 02:00:38 by isel-jao         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char haswall(t_mlx *m, double x, double y)
{
	if (in_map(m, x, y) && m->map.map[(int)x / 64] && m->map.map[(int)x / 64][(int)y / 64] != '1'
		// && m->map.map[(int)(x + 1) / 64]
		// && m->map.map[(int)(x + 1) / 64][(int)y / 64] != '1'
		// && m->map.map[(int)(x - 1) / 64][(int)y / 64] != '1'
		// && m->map.map[(int)x / 64][(int)(y + 1) / 64] != '1'
		// && m->map.map[(int)x / 64][(int)(y - 1) / 64] != '1'
	)

	{
		if (m->map.map[(int)x / 64][(int)y / 64])
